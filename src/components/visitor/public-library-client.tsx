  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6 px-4 py-8">
      <div>
        ...
        <p className="text-muted-foreground">Every take logged here, in one place.</p>
      </div>
        ...
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4">
          {groups.map(({ representative, count }) => (
            <RecordingCard
              ...
              href={
                count > 1
                  ? `/visitor/piece?title=${encodeURIComponent(representative.title)}&composer=${encodeURIComponent(representative.composer)}`
                  : undefined
              }
            />
          ))}
