...
export function VideoThumbnail({ recording }: { recording: Recording }) {
  return (
    <div className="group relative aspect-video w-full overflow-hidden rounded-lg bg-muted">
      {recording.videoSource === "YOUTUBE" && recording.youtubeId ? (
        // eslint-disable-next-line @next/next/no-img-element
        <img
          src={youtubeThumbnailUrl(recording.youtubeId)}
          alt=""
          className="h-full w-full object-cover"
        />
      ) : (
        <video
          src={`${recording.videoUrl}#t=0.5`}
          preload="metadata"
          muted
          className="h-full w-full object-cover"
        />
